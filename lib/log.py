...
class ColorFormatter(logging.Formatter):
  ...
  def _format(self, field, color): # pylint: disable=no-self-use
  ...
  def _format_quote(self, message, color):
    ...
    pieces = []
    for piece_pair in split_quoted(message, separator):
...
def inspect_object(obj, pretty=True, with_hidden=False):
  "Inspect an object and return information about its contents"
  values = []
  for attr in sorted(dir(obj)):
    if attr.startswith("_") and not with_hidden:
      continue
    value = getattr(obj, attr)
    if isinstance(value, (types.FunctionType, types.BuiltinFunctionType)):
      value_str = type(value).__name__
    else:
      value_str = f"[{type(value).__name__}]{value!r}"
    values.append(f"obj.{attr} = {value_str}")
  if pretty:
    return "\n".join(values)
  return ", ".join(values)

def serialize_one(value, pretty=False, **pretty_kwargs):
  """
  Return a string representation of the value. Will not modify the value in
  any way, unlike serialize_values.
  """
  if pretty:
    return pprint.pformat(value, **pretty_kwargs)
  return repr(value)

def serialize_values(value_list, pretty=False, **pretty_kwargs):
  """
  Return a list of strings, one for each value.

  Note that this function does modify the list in cases where examining the
  value would cause downstream problems, such as with generators.
  """
  results = []
  for idx in range(len(value_list)): # pylint: disable=consider-using-enumerate
    value = value_list[idx]
    if isinstance(value, types.GeneratorType):
      value_copy = list(value)
      value_list[idx] = (item for item in value_copy)
      results.append(serialize_one(value_copy, pretty=pretty, **pretty_kwargs))
    else:
      results.append(serialize_one(value, pretty=pretty, **pretty_kwargs))
  return results

# TODO: Formatting of keyword arguments
def traced_function(logger, when="before", trace_func="trace",
    context=False, pretty=False, **pretty_kwargs):
  """
  Build a decorator to trace function execution.

  Does not support infinite generators. Do not use this function if the traced
  function accepts or returns infinite generators.
  ...
  Arguments:
    ...
    pretty        If True, format the values through pprint

  Caveats:
    Note that in order to display generator content, this function does replace
    generators passed to and returned from the traced function with copies.
    This means that any intelligent generator processing will be performed
    immediately rather than as needed by the function or the calling code. This
    also means that infinite generators will give a MemoryError.

    There is not (yet) intelligent formatting for keyword arguments, and so
    keyword arguments are not modified in any way.
  ...
  def decorator(func):
    ...
    def wrapper(*args, **kwargs):
      "Function wrapper"
      arg_list = list(args) # make a copy for later modification
      name = func_name(func)
      ...
      args_strs = serialize_values(arg_list, pretty=pretty, **pretty_kwargs)
      ...
      if when in ("after", "both"):
        result_list = [result] # so serialize_values can modify it if needed
        result_strs = serialize_values(result_list, pretty=pretty, **pretty_kwargs)
        result = result_list[0]
        result_str = result_strs[0]
        do_trace("%s %s returned %s", prefix(), call_frame, result_str)
